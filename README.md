...
[![Tests](https://img.shields.io/badge/tests-220%20passed-2E7D32)](#测试)
...
当前测试覆盖数据库事务、并发分配、账号轮换、迁移与备份、DPAPI、macOS Keychain/AES-GCM、队列恢复、Web API、账号级独立 S5 与 SID、iCloud HME cURL/HAR、选择性 Alias 接管、母号归属与按需创建、已用完池、IMAP 精确收件与代理隔离、双账号网络隔离、BrowserForge 持久化、Chrome major 门禁、地域时区与 UTC 时钟一致性以及双目标推送。
