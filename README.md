...
| 一致身份 | 单次运行生成一个浏览器指纹和代理快照，并贯穿完整流程 |
| 双目标推送 | 工作流完成后可分别推送到 CPA Management 和 Sub2API |
| 加密备份 | 数据库快照、完整性校验、DPAPI 解密验证和受控恢复 |
| 本地控制台 | 仅绑定 `127.0.0.1`，写操作同时校验同源与启动期请求令牌 |

## 选择 CPA Management 或 Sub2API

根据现有使用反馈，同类账号通过 CPA 接入 BugTeam 时，可调用额度通常只有 `100` 美元出头；通过 Sub2API 接入时，可用额度接近或超过 `200` 美元。若优先考虑额度利用率，建议启用 Sub2API 推送，并将 CPA Management 保留为兼容或备用目标。

本项目目前只通过自动化回归测试，尚未在真实账号、真实额度和真实推送环境下完成端到端验证。上述额度差异不是本项目的实测结果，也不是固定额度承诺；实际数值可能随账号状态、上游策略和结算周期变化，请以服务端返回结果为准。
...
队列在每个阶段保存 checkpoint。进程异常退出后可以恢复同一次运行，避免重复邀请、重复退出或重复创建远端资源。
...
当前测试覆盖数据库事务、并发分配、账号轮换、迁移与备份、DPAPI、队列恢复、Web API、指纹一致性以及双目标推送。
