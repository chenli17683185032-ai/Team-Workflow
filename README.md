...
| OpenBrowser 接力 | 新子号永久绑定独立 OpenBrowser Profile，由用户在可见官方页面手工注册/登录；邮箱与 Team 校验后才继续生成凭据 |
...
5. 在 OpenBrowser 中预建空白 Profile 池，为每个 Profile 配好需要的代理与环境；在“设置 / OpenBrowser”保存回环 Local API 地址、API Key、Profile ID 池和人工登录时限，并确认连接检测显示可用容量。Team Workflow 不会创建、删除或重置 OpenBrowser Profile。
...
4. 正常轮换点击 Team 行的“一键换班”。控制台先确认 Local API 和空闲 Profile，再现场创建一个新 Alias并永久绑定该 Profile，随后严格执行“旧号登录 -> 校验成员并邀请 -> 旧号退出反馈 -> 打开 OpenBrowser 等待手工注册/登录 -> 邮箱与 Team 校验 -> 两人复核 -> PAT / CPA / Sub2”。母号不会参与正常换班。
...
7. 只有当前子号已经故障时才点击“提拉”。母号会登录 Team，逐人清退除母号外的成员并逐次读取远端确认；只剩母号 1 人后才邀请新子号并打开它绑定的 OpenBrowser Profile 等待手工登录，最终必须恰好为母号与新子号 2 人。
...
现场新 Alias 不再由 Team Workflow 自动填写注册页面。工作流只启动该账号永久绑定的 OpenBrowser Profile 并打开 ChatGPT 官方入口；邮箱、密码、OTP、资料、CAPTCHA、手机验证和条款均由用户在可见页面中处理。应用只读同一 Profile 的 ChatGPT session，严格核对登录邮箱和目标 Team；登录错号时保持窗口等待纠正，目标 Team 尚未出现时继续有界等待，关闭窗口、点击停止或超时都会终止且不会进入 PAT/JSON。失败和重试始终复用原 Profile，不清 Cookie、不换环境。
...
│  ├─ openbrowser.py        # 回环 Local API、Profile 选择与人工登录反馈
