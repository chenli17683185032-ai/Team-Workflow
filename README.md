...
[![Tests](https://img.shields.io/badge/tests-236%20passed-2E7D32)](#测试)
...
| iCloud 子号接力 | 一个 iCloud 资源池可接管多个永久 Team 母号；母号只做归属锚点，点击更换时才新建 Alias，实际接力全部由子号完成 |
| 双 LokiProxy 中继 | 两条工作流保留各自 LokiProxy 动态源，并统一先经过现有本地 Clash；不写入或 reload Mihomo 配置 |
...
可用 `TEAM_WORKFLOW_LOCAL_CLASH_PROXY` 覆盖两个 LokiProxy 中继共用的 Clash 前置地址；macOS 默认值为 `http://127.0.0.1:7897`。控制台不会写入或 reload Clash 配置。
...
在 iCloud 母号空间中，以上八个阶段的执行主体始终是当前子号和下一子号。母号仅作为空间归属、子号默认链路和 HME 资源池生成配置的锚点，不会被登录、邀请、退出或创建令牌。
...
| Team 母号 | 从已有 Alias 中接管，永久保护，只做归属锚点和生成配置，不登录、不邀请、不退出、不创建令牌，也不进入普通账号池、轮换或已用完池 |
...
4. 分别准备两个 LokiProxy 生成链接。它们对应工作流 A/B 各自的最终代理；新建子号会按归属继承对应本地中继地址，这不是母号登录代理。

#### 两个 LokiProxy 共用一个 Clash 前置

控制台为两个母号各自创建一个仅绑定 `127.0.0.1` 的轻量 SOCKS5 中继。两条中继都先通过本机现有 Clash 连接各自 LokiProxy，再由 LokiProxy 访问目标：

```text
工作流 A -> 本地中继 A -> Clash 127.0.0.1:7897 -> LokiProxy A -> 目标
工作流 B -> 本地中继 B -> Clash 127.0.0.1:7897 -> LokiProxy B -> 目标
```

在“同步 Alias”或已接管母号的“子号默认链路”对话框中，分别填写两个 LokiProxy 生成链接。页面中的“统一 Clash 前置”是同一个只读地址，默认是 `http://127.0.0.1:7897`，不能给 A/B 各选一个 Clash 节点。

生成 API 请求和到动态 LokiProxy 端点的连接都会经过这个统一前置，因此基于 Clash 出口白名单的 LokiProxy 可以正常访问。需要切换 Clash 出口时直接在 Clash 中操作；控制台不创建第二个 Clash，不写入 listener/provider/dialer-proxy，不调用 reload，也不覆盖选择器。
...
5. 为两个 Team 母号分别保存各自的 LokiProxy 生成链接；二者的“统一 Clash 前置”保持相同。页面不会回显生成链接或动态端点凭据，这一步不会登录或操作母号。
6. 新建或编辑两个空间：每个空间选择一个 Team 母号和它当前的子号，下一子号保持为空。
7. 需要轮换时，在空间行点击“更换子号”。控制台此时才通过 HME 资源池创建一个全新 Alias，按空间母号绑定并继承子号默认链路，然后立即加入接力队列；母号没有任何 Team 动作。
8. 接力成功后，新子号晋升为当前子号，旧子号进入“已用完”池，空间重新回到无下一子号状态。下一次点击会再次创建一个新 Alias。

本地直连测试只验证网络可达性，不会自动延长 Apple Session。Session 仍需在同一 `icloud.com.cn` 登录态下重新捕获最新 HME 请求；如果直连后仍立即失效，应优先检查 Apple 登录状态和 Cookie，而不是反复点击检测。

若 Apple 已创建并完成本地绑定，但任务暂时未能入队，空间会保留这个下一子号并显示“开始接力”；再次点击只会入队，不会重复创建 Alias。之后仍可在账号行单独覆盖新子号代理；任务开始后使用冻结的账号代理快照。

编辑资源池时，秘密输入留空表示保留已保存值，页面不会回填 Cookie、IMAP 密码或代理。更换 Session、IMAP、转发邮箱或资源池 S5 后必须重新检测；子号默认链路在 Alias 列表中独立维护。
...
当前测试覆盖数据库事务、并发分配、账号轮换、迁移与备份、DPAPI、macOS Keychain/AES-GCM、队列恢复、Web API、账号级独立 S5 与 SID、iCloud HME cURL/HAR、选择性 Alias 接管、母号归属与按需创建、已用完池、IMAP 精确收件与代理隔离、双账号网络隔离、统一 Clash 前置、双 LokiProxy 字节流中继、TTL 缓存与并发隔离、BrowserForge 持久化、Chrome major 门禁、地域时区与 UTC 时钟一致性以及双目标推送。
