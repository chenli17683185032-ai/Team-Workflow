...
[![Tests](https://img.shields.io/badge/tests-215%20passed-2E7D32)](#测试)
...
| iCloud 隐藏邮箱 | 每个 iCloud 母号独立保存 HME 会话、真实转发邮箱 IMAP 和 S5，生成的 Alias 直接进入账号池 |
...
2. **账号库**：管理 Outlook 库存、iCloud 隐藏邮箱母号和已生成的普通账号池。
...
- Python 3.11 或更高版本
- `curl-cffi 0.15.0`、`Playwright 1.58.0`、`BrowserForge 1.2.4`、`cryptography 43+`
...
| iCloud 母号与 Alias | 应用数据库内的独立表 | HME Cookie、IMAP 密码、母号 S5 和远端 ID 分用途认证加密 |
...
本版本把 iCloud Hide My Email 接入现有账号池。每个母号分别配置 HME 网页会话、真实转发邮箱 IMAP 和独立 S5；HME 请求与 IMAP 收码都使用该母号 S5，不会借用全局代理。生成成功的隐藏邮箱是普通 `available` 账号，可直接绑定为空间的当前账号或下一账号。
...
3. 准备该母号自己的 S5。建议使用 `socks5h://user:password@host:port`，让 IMAP 主机名也由代理端解析。

HAR 可能包含大量会话信息，不要保存到仓库。控制台只解析允许的 Apple HME host/path，导入原文不落盘；提取后的 Cookie 会立即进入系统密钥保护的密文。

### 控制台操作

1. 打开“账号库”中的“iCloud 隐藏邮箱”，点击“添加母号”。
2. 填写母号名称、真实转发邮箱、HME cURL/HAR、IMAP 参数和母号 S5，然后保存。
3. 点击“检测”。只有 HME Session、转发目标和 IMAP 登录同时通过，母号状态才会变为“可用”。
4. 点击“生成”，一次创建 1 到 20 个 Alias。每个 Alias 由 iCloud `generate` 后再 `reserve`，成功一个就立即写入账号池；批次中途失败不会回滚已成功项。
5. 返回“账号池”，来源为 `iCloud HME` 的账号可直接参与原有绑定、轮换和队列流程。

母号 S5 会在 Alias 创建时复制为新账号的默认账号代理，因此每个母号可以使用完全不同的出口。之后仍可在账号行的“代理”操作中单独覆盖；任务开始后继续使用冻结的账号代理快照。

编辑母号时，秘密输入留空表示保留已保存值，页面不会回填 Cookie、IMAP 密码或代理。更换 Session、IMAP、转发邮箱或母号 S5 后必须重新检测；只改母号名称不会中断已通过检测的账号。

> [!WARNING]
> Hide My Email Web API 不是 Apple 官方稳定 API，Cookie 也会过期。出现“Session 失效”时，重新从已登录的 iCloud 页面复制最新 cURL/HAR，更新母号并再次检测。控制台不会自动删除远端 Alias，也不会在未点击“生成”时创建 Alias。
...
当前测试覆盖数据库事务、并发分配、账号轮换、迁移与备份、DPAPI、macOS Keychain/AES-GCM、队列恢复、Web API、账号级独立 S5 与 SID、iCloud HME cURL/HAR、加密母号与 Alias、IMAP 精确收件与代理隔离、双账号网络隔离、BrowserForge 持久化、Chrome major 门禁、地域时区与 UTC 时钟一致性以及双目标推送。
