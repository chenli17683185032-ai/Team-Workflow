...
[![Tests](https://img.shields.io/badge/tests-253%20passed-2E7D32)](#测试)
...
| 双 LokiProxy 中继 | 两条工作流各自使用 LokiProxy 生成源或固定 SOCKS5，并统一先经过现有本地 Clash；不写入或 reload Mihomo 配置 |
...
4. 分别准备两个 LokiProxy 生成链接或完整的固定 `socks5://user:password@host:port` 端点。它们对应工作流 A/B 各自的最终代理；新建子号会按归属继承对应本地中继地址，这不是母号登录代理。
...
在“同步 Alias”或已接管母号的“子号默认链路”对话框中，分别填写两个 LokiProxy 生成链接或固定 SOCKS5 端点。页面中的“统一 Clash 前置”是同一个只读地址，默认是 `http://127.0.0.1:7897`，不能给 A/B 各选一个 Clash 节点。
...
2. 填写资源池名称、真实转发邮箱、IMAP 参数和可选的 HME / IMAP S5，然后保存；HME Session 可留空，之后点击“登录更新 HME”自动捕获，也可粘贴手动 cURL/HAR。
