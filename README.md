...
根据当前实测，同类账号通过 CPA 接入 BugTeam 时，可调用额度通常只有 `100` 美元出头；通过 Sub2API 接入时，可用额度接近或超过 `200` 美元。若优先考虑额度利用率，建议启用 Sub2API 推送，并将 CPA Management 保留为兼容或备用目标。

该差异来自当前上游计量与适配表现，不是本项目承诺的固定额度。实际数值可能随账号状态、上游策略和结算周期变化，请以服务端返回结果为准。
