...
[![Tests](https://img.shields.io/badge/tests-390%20total-2E7D32)](#测试)
...
| OpenBrowser 接力 | 新子号永久绑定独立 OpenBrowser Profile，从官方 `Log in` 入口自动填邮箱、读取 OTP 并按需补全姓名；邮箱与 Team 校验后才继续生成凭据 |
| iCloud 子号接力 | 一个 iCloud 资源池可接管多个永久 Team 母号；正常换班全部由子号完成，当前子号故障时才允许母号执行独立的应急提拉 |
| Team 两跳代理 | 每个 Team 可直接粘贴 CliProxy 等固定 HTTP/SOCKS 链接，并统一先经过现有本地 Clash；支持受限 Linkup RPAPI 与历史 Loki 生成源，不写入或 reload Mihomo 配置 |
...
4. **设置**：配置 OpenBrowser、代理、PAT、推送目标、输出目录以及加密备份。

```text
邮箱库存 ──按需分配──> 空间账号对 ──加入队列──> 单任务执行器
                                               │
                                               ├─ 旧/新账号各自固定 SID、代理与指纹
                                               ├─ 登录 / 邀请 / 退出 / OpenBrowser 自动接力
                                               ├─ PAT 与 CPA 生成
...
- Python 3.11 或更高版本
- [OpenBrowser](https://github.com/lyu0805/OpenBrowser) `v1.0.2` 或更高版本，Local API 仅监听回环地址
...
| OpenBrowser 新号 profile | OpenBrowser 自己的本地数据目录 | 一个 Profile 永久绑定一个 OpenAI 账号；Team Workflow 不读取池外 Profile，不清理 Cookie，也不把该目录纳入项目备份 |
...
4. 分别准备每个 Team 的代理。可以粘贴完整的 `http://user:password@host:port` / SOCKS5 URL，也可以直接粘贴供应商给出的 `curl -L -x HOST:PORT -U "USER:PASS" TARGET` 或 `curl --socks5 HOST:PORT -U "USER:PASS" TARGET`；精确的 Linkup 固定入口 `global.rp.linkup.onl` 会按现场协议要求解释为 SOCKS5，其他 `-x` 仍为 HTTP。受限的 `global.rpapi.linkup.onl/gen` 与历史 Loki `/gen` 链接同样支持。生成请求和生成后的代理连接都必须经过统一 Clash 第一跳，不会直连；curl 文本只做受限参数解析，不会执行，探针目标也不会保存。新建子号会按归属继承对应本地中继地址，这不是母号登录代理。
5. 在 OpenBrowser 中预建空白 Profile 池，为每个 Profile 配好需要的代理与环境；在“设置 / OpenBrowser”保存回环 Local API 地址、API Key、Profile ID 池和自动登录时限，并确认连接检测显示可用容量。Team Workflow 不会创建、删除或重置 OpenBrowser Profile。
...
4. 正常轮换点击 Team 行的“一键换班”。控制台先确认 Local API 和空闲 Profile，再现场创建一个新 Alias 并永久绑定该 Profile，随后严格执行“旧号登录 -> 校验成员并邀请 -> 旧号退出反馈 -> OpenBrowser 从 `Log in` 自动登录 -> 邮箱与 Team 校验 -> 两人复核 -> PAT / CPA / Sub2”。母号不会参与正常换班。
...
7. 只有当前子号已经故障时才点击“提拉”。母号会登录 Team，逐人清退除母号外的成员并逐次读取远端确认；只剩母号 1 人后才邀请新子号，并在它绑定的 OpenBrowser Profile 中自动登录，最终必须恰好为母号与新子号 2 人。
...
现场新 Alias 的新号阶段从 ChatGPT 首页点击官方 `Log in` 开始，不点击 `Sign up` 也不跳过首页直接进入注册链。程序在该账号永久绑定的 OpenBrowser Profile 中自动提交 Alias，到达邮箱验证页后从其 iCloud IMAP 读取新 OTP 并填入；出现唯一可见姓名控件时自动填写生成的姓名。未配置密码却出现强制密码页、CAPTCHA、手机验证、条款/年龄确认、控件歧义或未知页面时都会停止，不做绕过。

应用只使用同一 Profile 的 context/page 与 ChatGPT session，严格核对登录邮箱和目标 Team。Profile 已有正确账号 session 时不重发 OTP；已有其他账号时立即停止，不自动退出或清 Cookie。目标 Team 尚未出现时继续有界等待；关闭窗口、点击停止或超时都会终止且不会进入 PAT/JSON。失败和重试始终复用原 Alias/Profile，不清 Cookie、不换环境。

OpenBrowser 提供 Cookie、缓存和本地存储隔离，但不能保证指纹唯一、匿名或避免服务端账号关联与限制。不要把 Profile 隔离理解为规避平台风控；Team Workflow 不会绕过 CAPTCHA、手机验证、账号停用或平台策略。
...
当保存的 browser cookie 仍能交换会话、但 PAT 端点明确返回 `401 token_invalidated` 时，当前子号刷新会清除该失效会话，并在不导入旧 Cookie 的新登录环境中执行一次 OTP 重登。其他 401、403、429、5xx 或超时不会触发该回退，第二次 PAT 失败也不会继续循环。
...
│  ├─ openbrowser.py        # 回环 Local API、Profile 选择与自动登录反馈
...
当前测试覆盖数据库事务、并发分配、账号轮换、迁移与备份、DPAPI、macOS Keychain/AES-GCM、队列恢复、Web API、账号级独立 S5 与 SID、OpenBrowser 回环 API、Profile 永久绑定与池耗尽、自动 `Log in` 入口、邮箱 OTP、姓名补全、错号停止、Team 等待、窗口关闭、取消、超时和单 Profile 停止、iCloud HME cURL/HAR、登录后 HME 自动捕获状态机、按资源池隔离的持久登录 profile、Workspace 自动识别与两人唯一匹配、选择性 Alias 接管、幂等 Team 导入、母号归属与按需创建、已用完池、IMAP 精确收件与代理隔离、Sub2API 告警 UID 消费与动作幂等、失败下一子号 JSON 恢复、当前子号 PAT 刷新、刷新幂等与详细日志、子号 browser cookie 生命周期、正常子号换班、外部 iCloud 晋升、母号应急提拉、逐人清退反馈、Team 两人硬上限、无关待邀请阻断、退出前成员反馈、入组后成员反馈、双账号网络隔离、统一 Clash 第一跳、固定 HTTP/SOCKS 与受限 curl 命令输入、Linkup RPAPI 与历史动态源、JSON 缺省协议、TTL 缓存与并发隔离、BrowserForge 持久化、Chrome major 门禁、地域时区与 UTC 时钟一致性、PAT + Session 的 Sub2API 导出、私有原子文件恢复以及双目标可选推送。最新测试总数和平台跳过数以本次发布的全量测试结果为准。
